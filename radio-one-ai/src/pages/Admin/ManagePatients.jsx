...
export default function ManagePatients() {
  ...
  const handleViewDetails = (patient) => {
    ...
    setTimeout(() => { document.getElementById("view_patient_modal").showModal(); }, 0);
  };
    ...
    <div ref={container} className="space-y-8 p-4">
      ...
      <div className="page-header flex justify-between items-center">
        <div>
          ...
          <p className="text-base-content/50 font-medium">Overview of registered patients and clinical history.</p>
        </div>
              ...
              {isLoadingPatients ? (
                ...
                filteredPatients.map((pt) => (
                  <tr key={pt.id} className="hover:bg-base-200/50 transition-colors group border-b border-base-content/5 last:border-0">
                    ...
                    <td><span className="font-mono font-black text-xs opacity-50">{pt.registeredDate}</span></td>
                    <td>
                    ...
                    <td className="px-8 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => handleViewDetails(pt)} className="btn btn-ghost btn-xs rounded-lg font-black hover:bg-base-300">VIEW</button>
                        <button className="btn btn-ghost btn-xs rounded-lg font-black text-error hover:bg-error/10">BAN</button>
                      </div>
                    </td>
                  </tr>
                ))
