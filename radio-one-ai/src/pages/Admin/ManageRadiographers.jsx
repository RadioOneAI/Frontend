...
export default function ManageRadiographers() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setRadiographers(list.map(mapApiUserToUi));
    } catch (error) {
      setToast({
        message: error.message || "Unable to fetch radiographers.",
        type: "error",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleViewDetails = (r) => {
    ...
    setTimeout(() => {
      document.getElementById("view_radiographer_modal").showModal();
    }, 0);
  };
  ...
  return (
    <div ref={container} className="space-y-8 p-4">
      ...
      <div className="page-header flex justify-between items-center">
        <div>
          <h1 className="text-4xl font-black tracking-tight mb-2">
            Manage <span className="text-gradient">Radiographers</span>
          </h1>
          <p className="text-base-content/50 font-medium">
            Manage technical staff responsible for image acquisition.
          </p>
        </div>
              ...
              {isLoading ? (
                ...
                filtered.map((r) => (
                  ...
                  >
                    ...
                    <td>
                      <span className="font-mono font-black text-xs text-accent bg-accent/5 px-3 py-1.5 rounded-lg border border-accent/10">
                        {r.licenseId}
                      </span>
                    </td>
                    <td>
                    ...
                    <td className="px-8 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button className="btn btn-ghost btn-xs rounded-lg font-black text-error hover:bg-error/10 border border-error/30 hover:border-error/60">
                          DELETE
                        </button>
                      </div>
                    </td>
      ...
      <RadiographerDetailsModal
        radiographer={selectedRadiographer}
        onClose={() => setSelectedRadiographer(null)}
      />
    </div>
  );
}
