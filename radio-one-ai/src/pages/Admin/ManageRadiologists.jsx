...
export default function ManageRadiologists() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (!res.ok || json?.success === false) throw new Error(json?.message || "Failed to load radiologists.");
      const list = Array.isArray(json?.data) ? json.data : [];
      setRadiologists(list.map(mapApiUserToUi));
    } catch (error) {
      setToast({ message: error.message || "Unable to fetch radiologists.", type: "error" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleViewDetails = (r) => {
    ...
    setTimeout(() => { document.getElementById("view_radiologist_modal").showModal(); }, 0);
  };
    ...
    <div ref={container} className="space-y-8 p-4">
      ...
      <div className="page-header flex justify-between items-center">
        <div>
          ...
          <p className="text-base-content/50 font-medium">Manage imaging specialists and their system access.</p>
        </div>
              ...
              {isLoading ? (
                ...
                filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-base-200/50 transition-colors group border-b border-base-content/5 last:border-0">
                    ...
                    <td><span className="font-mono font-black text-xs text-secondary bg-secondary/5 px-3 py-1.5 rounded-lg border border-secondary/10">{r.regNo}</span></td>
                    <td>
