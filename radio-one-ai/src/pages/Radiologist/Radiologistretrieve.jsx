...
export default function Radiologistretrieve() {
  ...
  useGSAP(
    () => {
      ...
      gsap.from(".stat-card", { y: 20, opacity: 0, stagger: 0.1, duration: 0.8, ease: "power3.out" });
  ...
  useEffect(() => {
    const fetchPrescriptions = async () => {
      ...
      try {
        ...
        setToast({ message: error.message || "Unable to load prescriptions.", type: "error" });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const openPdfReportModal = async (a) => {
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok || json?.success === false) throw new Error(json?.message || "Failed to load report details.");
      const report = extractReportFromPayload(json, reportId);
      if (!report) throw new Error("Report details not found.");
      
      setPdfReportState({ loading: false, error: "", requestedId: reportId, resolvedId: report.id ?? null, report });
    } catch (error) {
    ...
    <div ref={container} className="space-y-8 p-4">
      ...
      <div className="page-header flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          ...
          <p className="text-base-content/50 font-medium italic">Manage diagnostic workflows, AI analysis, and finalized reports.</p>
        </div>
          ...
          <table className="table table-zebra w-full text-center">
            <thead>
              <tr className="text-base-content/40 uppercase tracking-widest text-[10px] font-black border-b border-base-content/5">
                ...
                <th className="py-6">Assigned Staff</th>
            ...
            <tbody className="font-bold text-sm">
              {isLoading ? (
                ...
                filteredAppointments.map((a) => (
                  <tr key={a.requestId} className="hover:bg-base-200/50 transition-colors group border-b border-base-content/5 last:border-0">
                    <td className="py-5 px-8 text-left">
                       <div className="font-black text-primary text-base">{a.requestId}</div>
                       <div className="text-[10px] font-bold opacity-40 uppercase tracking-widest">{a.createdAt}</div>
                    </td>
                    <td><span className="font-black text-base-content/80 text-base">{a.patient}</span></td>
                    <td>
                       <div className="font-black">{a.scanType}</div>
                       <div className="text-[10px] font-bold opacity-40 uppercase tracking-widest">{a.organ}</div>
                    </td>
                    <td>
                       <div className="font-bold text-base-content/70">{a.doctor}</div>
                       <div className="text-[10px] opacity-40 uppercase tracking-widest">{a.radiographer}</div>
                    </td>
                    <td>
                      <div className={`badge badge-md font-black px-4 py-3 rounded-xl border-none uppercase text-[10px] shadow-lg ${
                        (a.analysis || String(a.status).toLowerCase().includes("completed") || String(a.status).toLowerCase().includes("ready"))
                          ? "badge-secondary text-white shadow-secondary/10"
                          : "badge-success text-white shadow-success/10"
                      }`}>
                        {(a.analysis || String(a.status).toLowerCase().includes("completed") || String(a.status).toLowerCase().includes("ready")) ? "REPORT READY" : "PENDING"}
                      </div>
                    </td>
                ...
                <tr><td colSpan="6" className="py-20 text-center opacity-30 font-black uppercase tracking-[0.3em] text-xs">No appointments found</td></tr>
              )}
            </tbody>
          </table>
        </div>
