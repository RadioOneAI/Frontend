...
const METRICS = [
  { key: "fid", label: "FID", dir: "lower better", cgan: 223.78, dcgan: 133.47, sd: 101.53 },
  { key: "is", label: "Inception Score", dir: "higher better", cgan: 1.63, dcgan: 1.93, sd: 2.45 },
  { key: "sharp", label: "Sharpness", dir: "higher better", cgan: 1419.46, dcgan: 1589.39, sd: 3362.04 },
  { key: "edge", label: "Edge Density", dir: "higher better", cgan: 0.1275, dcgan: 0.1814, sd: 0.1838 },
  { key: "dyn", label: "Dynamic Range", dir: "higher better", cgan: 128.85, dcgan: 138.56, sd: 187.32 },
  { key: "hist", label: "Histogram Corr.", dir: "higher better", cgan: 0.8322, dcgan: 0.6062, sd: 0.8739 },
];
        ...
        return (
          <div key={r.method} className="space-y-1">
            <div className="flex items-center justify-between text-sm">
              <span className="font-bold">#{r.rank} {r.method}</span>
              <span className="font-mono">{r.score.toFixed(4)}</span>
            </div>
            <progress
              className={`progress w-full ${r.rank === 1 ? "progress-success" : r.rank === 2 ? "progress-info" : "progress-error"}`}
              value={pct}
              max="100"
            />
    ...
    <div className="space-y-8">
      <section className="hero bg-base-200 rounded-3xl border border-base-300">
        <div className="hero-content flex-col lg:flex-row gap-8 py-12">
          <div className="max-w-2xl">
            <div className="badge badge-primary badge-outline mb-4">RadioSynth - Synthetic Medical Imaging</div>
            <h1 className="text-3xl md:text-5xl font-black leading-tight">
              Overcome tough data challenges with <span className="text-primary">RadioSynth</span>
            </h1>
            <p className="py-5 text-base-content/75">
              High-quality synthetic brain MRI generation for realistic, sharp, anatomically faithful datasets.
            </p>
          ...
          <div className="grid grid-cols-2 gap-3 w-full max-w-md">
            {HERO_IMAGES.map((src, i) => (
              <div key={src} className="rounded-2xl overflow-hidden border border-base-300 shadow">
                <img src={src} alt={`Synthetic brain MRI ${i + 1}`} className="h-28 w-full object-cover" loading="lazy" />
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {VALUE_TILES.map((tile) => (
          <div key={tile.label} className="card bg-base-100 border border-base-300 shadow-sm">
      ...
      <section className="card bg-base-100 border border-base-300 shadow-sm" id="sv-comparison">
        <div className="card-body">
          <h2 className="card-title">Method comparison</h2>
          <p className="text-base-content/70">RadioSynth wins all six evaluation metrics.</p>
                  ...
                  {METRICS.map((m) => {
                    ...
                    return (
                      <tr key={m.key}>
                        ...
                        <td className="text-xs uppercase tracking-wide text-base-content/60">{m.dir}</td>
                        <td>
                        ...
                        <td>
                          <div className="space-y-1 min-w-32">
                            <span className="text-sm font-bold text-success">{m.sd}</span>
      ...
      <section className="card bg-base-100 border border-base-300 shadow-sm">
        <div className="card-body">
          ...
          <p className="text-base-content/70">Generated by RadioSynth for downstream MRI AI tasks.</p>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-2">
            {SAMPLES.map((src, i) => (
              <div key={src} className="rounded-xl overflow-hidden border border-base-300">
                <img src={src} alt={`Synthetic MRI sample ${i + 1}`} className="h-36 w-full object-cover" loading="lazy" />
                <div className="px-3 py-2 text-xs text-base-content/70">Synthetic #{String(i + 1).padStart(2, "0")}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card-body">
          ...
          <div className="grid md:grid-cols-2 gap-4 mt-1">
            {METHODOLOGY.map((item) => (
              <div key={item.title} className="rounded-xl border border-base-300 p-4 bg-base-200/40">
