...
export default function RadiographerDetailsModal({ radiographer, onClose }) {
  ...
  useGSAP(
    ...
    { scope: container, dependencies: [radiographer] }
  );

  if (!radiographer) return null;

  const initials = radiographer.name
    ? radiographer.name.split(" ").map((n) => n[0]).join("").toUpperCase().substring(0, 2)
    : "RG";

  return (
    <dialog id="view_radiographer_modal" className="modal overflow-hidden">
      <div ref={container} className="modal-box w-11/12 max-w-4xl p-0 rounded-[3rem] border-none bg-white shadow-2xl overflow-hidden relative">
        ...
        <div className="flex flex-col lg:flex-row min-h-[550px]">
          <div className="lg:w-[40%] p-12 flex flex-col items-center justify-center relative profile-section bg-linear-to-b from-base-100 to-base-200/30">
            <div className="w-48 h-48 rounded-full border-[6px] border-white shadow-2xl bg-amber-50 flex items-center justify-center mb-8 relative group">
               <span className="text-6xl font-medium text-slate-700 tracking-tighter">{initials}</span>
            </div>
            <h3 className="text-4xl font-black tracking-tighter text-slate-900 text-center mb-4 leading-tight">{radiographer.name}</h3>
            <div className="bg-red-900 text-white font-black uppercase text-[11px] tracking-[0.2em] px-6 py-2 rounded-full mb-12 shadow-lg shadow-red-900/20">
              {radiographer.role || "RADIOGRAPHER"}
            </div>
            <div className="w-full max-w-[240px] bg-white rounded-3xl p-6 shadow-xl shadow-slate-200/50 border border-slate-50 text-center">
                ...
                <div className="text-[13px] font-black uppercase text-emerald-500 tracking-wider">
                   {radiographer.status === 'Active' ? 'ACTIVE ACCOUNT' : 'INACTIVE ACCOUNT'}
                </div>
            </div>
          </div>
          ...
          <div className="flex-1 p-12 lg:p-20 details-section space-y-12">
            <div className="space-y-8">
              <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-300 flex items-center gap-4">Staff Credentials<span className="flex-1 h-px bg-slate-100" /></h4>
              <div className="grid grid-cols-2 gap-12">
                <div className="space-y-2">
                  <span className="text-[11px] font-black text-slate-300 uppercase tracking-widest">License ID</span>
                  <p className="font-black text-2xl text-red-800 tracking-tighter">{radiographer.licenseId || 'N/A'}</p>
                </div>
                <div className="space-y-2">
                  <span className="text-[11px] font-black text-slate-300 uppercase tracking-widest">Hospital Branch</span>
                  <p className="font-black text-2xl text-slate-700 tracking-tighter">{radiographer.branch || 'Main Hospital'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-12">
                <div className="space-y-2">
                  ...
                  <p className="font-black text-2xl text-slate-700 tracking-tighter">{radiographer.phone || 'N/A'}</p>
                </div>
                <div className="space-y-2">
                  <span className="text-[11px] font-black text-slate-300 uppercase tracking-widest">Email Address</span>
                  <p className="font-black text-2xl text-slate-700 tracking-tighter truncate">{radiographer.email || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-12">
                <div className="space-y-2">
                  ...
                  <p className="font-black text-2xl text-slate-400 tracking-tighter italic">@{radiographer.staffId || 'N/A'}</p>
                </div>
                <div className="space-y-2">
                  <span className="text-[11px] font-black text-slate-300 uppercase tracking-widest">Account Created</span>
                  <p className="font-black text-2xl text-slate-400 tracking-tighter">{radiographer.created_at ? new Date(radiographer.created_at).toLocaleDateString() : 'N/A'}</p>
                </div>
              </div>
            </div>
