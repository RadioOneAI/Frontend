...
export default function ManageRadiographers() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (!res.ok || json?.success === false) throw new Error(json?.message || "Failed to load radiographers.");
      const list = Array.isArray(json?.data) ? json.data : [];
      setRadiographers(list.map(mapApiUserToUi));
    } catch (error) {
      setToast({ message: error.message || "Unable to fetch radiographers.", type: "error" });
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div ref={container} className="space-y-8 p-4">
      ...
      <div className="page-header flex justify-between items-center">
        <div>
          ...
          <p className="text-base-content/50 font-medium">Manage technical staff responsible for image acquisition.</p>
        </div>
