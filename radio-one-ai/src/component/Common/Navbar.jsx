...
export default function Navbar({
  ...
  try {
    ...
    if (rawUser) {
      ...
      localUserName = parsedUser?.name || parsedUser?.username || "";
    }
  } catch (error) {
    localUserName = "";
  }

  const displayName = localUserName || user.username || "User Account";
  ...
  return (
    <div
      ref={container}
      className="sticky top-0 z-40 p-4 md:p-6 pb-0 pointer-events-none"
    >
      <div className="nav-content glass-card h-20 px-8 rounded-[2rem] border border-white/10 shadow-2xl shadow-primary/5 flex items-center justify-between pointer-events-auto relative overflow-hidden group">
        ...
        <div className="flex-none lg:hidden">
          <label
            htmlFor={drawerId}
            className="btn btn-square btn-ghost rounded-2xl hover:bg-primary/10 hover:text-primary transition-all"
          >
            <svg
        ...
        <div className="flex-1 px-4">
          <div className="flex flex-col">
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-primary mb-1 animate-pulse">
              Live Session
            </span>
        ...
        <div className="flex items-center gap-6">
          ...
          >
            {theme === "dark" ? (
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-5 w-5 text-warning fill-warning/20"
                viewBox="0 0 24 24"
                stroke="currentColor"
                fill="none"
              >
          ...
          <div className="flex items-center gap-4 pl-6 border-l border-base-content/10">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-black text-base-content/90 tracking-tight leading-none mb-1.5">
                {displayName}
              </p>
